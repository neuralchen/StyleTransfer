...
# -*- coding:utf-8 -*-
#############################################################
# File: Discriminator_VGG.py
# Created Date: Friday April 10th 2020
# Author: Chen Xuanhong
# Email: chenxuanhongzju@outlook.com
# Last Modified:  Saturday, 11th April 2020 12:29:42 am
...
class Discriminator(nn.Module):
    def __init__(self, chn=32, k_size=5):
        ...
        self.block0 = nn.Sequential(
            nn.Conv2d(in_channels = 3 , out_channels = chn, kernel_size= 3, stride = 1, bias= False),
            nn.InstanceNorm2d(chn, affine=True, momentum=0),
            nn.LeakyReLU(slop),
            nn.Conv2d(in_channels = chn , out_channels = chn*2, kernel_size= 3, stride = 2, bias= False),
        ...
        self.aux_classfier0 = nn.Sequential(
            ...
            nn.Conv2d(in_channels = chn * 2 , out_channels = chn * 2, kernel_size= 3, stride = 1, bias= False),
            nn.InstanceNorm2d(chn, affine=True, momentum=0),
            nn.LeakyReLU(slop),
            nn.Conv2d(in_channels = chn * 2 , out_channels = chn*2, kernel_size= 3, stride = 1, bias= False)
        )
        ...
        self.block1 = nn.Sequential(
            ...
            nn.InstanceNorm2d(chn, affine=True, momentum=0),
            nn.LeakyReLU(slop),
            nn.Conv2d(in_channels = chn * 2 , out_channels = chn * 2, kernel_size= 3, stride = 2, bias= False),
            nn.InstanceNorm2d(chn * 2 , affine=True, momentum=0),
            nn.LeakyReLU(slop)
        )
        ...
        self.block6 = nn.Sequential(
            nn.Conv2d(in_channels = chn * 16 , out_channels = chn * 16, kernel_size= 3, stride = 1, bias= False),
            nn.InstanceNorm2d(chn * 16, affine=True, momentum=0),
            nn.LeakyReLU(slop),
            nn.Conv2d(in_channels = chn * 16 , out_channels = chn * 16, kernel_size= 3, stride = 2, bias= False),
            nn.InstanceNorm2d(chn * 16 , affine=True, momentum=0),
            nn.LeakyReLU(slop)
        )
