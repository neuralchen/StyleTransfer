...
class Discriminator(nn.Module):
    def __init__(self, chn=32, k_size=5,n_class=3):
        super().__init__()
        ...
        self.aux_classfier0 = nn.Sequential(
            nn.Conv2d(in_channels = chn * 2 , out_channels = n_class+1 , kernel_size= 5),
        )
        ...
        self.aux_classfier1 = nn.Sequential(
            nn.Conv2d(in_channels = chn * 2 , out_channels = n_class+1, kernel_size= 10),
        )
        ...
        self.aux_classfier3 = nn.Sequential(
            nn.Conv2d(in_channels = chn * 8 , out_channels = n_class+1, kernel_size= 10),
        )
        ...
        self.aux_classfier5 = nn.Sequential(
            nn.Conv2d(in_channels = chn * 16 , out_channels = n_class+1, kernel_size= 6),
        )
