...
class Discriminator(nn.Module):
    def __init__(self, chn=32, k_size=3):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.LeakyReLU(slop),
            utils.spectral_norm(nn.Conv2d(in_channels = chn, out_channels = chn * 2 , kernel_size= k_size, stride = 2, bias= enable_bias)), # 1/4
            nn.LeakyReLU(slop)
        )
        ...
        self.linear1 = utils.spectral_norm(nn.Linear(chn, 11))

        # stage 2
        self.block2 = nn.Sequential(
            utils.spectral_norm(nn.Conv2d(in_channels = chn * 2 , out_channels = chn * 4 , kernel_size= k_size, stride = 2, bias= enable_bias)),# 1/8
            nn.LeakyReLU(slop),
            utils.spectral_norm(nn.Conv2d(in_channels = chn * 4, out_channels = chn * 8 , kernel_size= k_size, stride = 2, bias= enable_bias)),# 1/16
            nn.LeakyReLU(slop)
        )
        ...
        self.linear2 = utils.spectral_norm(nn.Linear(chn, 11))

        # stage 3
        self.block3 = nn.Sequential(
            utils.spectral_norm(nn.Conv2d(in_channels = chn * 8 , out_channels = chn * 8 , kernel_size= k_size, stride = 2, bias= enable_bias)),# 1/32
            nn.LeakyReLU(slop),
            utils.spectral_norm(nn.Conv2d(in_channels = chn * 8, out_channels = chn * 16 , kernel_size= k_size, stride = 2, bias= enable_bias)),# 1/64
            nn.LeakyReLU(slop)
        )
        ...
        self.linear3 = utils.spectral_norm(nn.Linear(chn, 11))
