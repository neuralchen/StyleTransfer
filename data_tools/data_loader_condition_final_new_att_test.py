...
class data_prefetcher():
    ...
    def preload(self):
        try:
            self.style, self.label = next(self.dataiter)
...
class StyleDataset(data.Dataset):
    ...
    def __init__(self,style_image_dir,selectedStyle,style_transform,
                    subffix='jpg', random_seed=1234,mode='train'):
        ...
        self.art_dataset_train   = []
        self.art_dataset_test    = []
        self.random_seed= random_seed
        self.mode =mode
        self.preprocess()
        #self.num_images = len(self.content_dataset)
        self.art_num    = len(self.art_dataset_test if self.mode == 'test' else self.art_dataset_train)
    ...
    def preprocess(self):
        ...
        for class_item in self.selectedStyle:
            ...
            images_num=sum(1 for _ in images)
            print(images_num)
            images = Path(self.style_image_dir).glob('%s/*.%s'%(class_item, self.subffix))
            for ii,item in enumerate(images):
                if ii<(images_num//10):
                    self.art_dataset_test.append([item, label_index])
                else:
                    self.art_dataset_train.append([item, label_index])
            label_index += 1
        random.seed(self.random_seed)
        #random.shuffle(self.content_dataset)
        random.shuffle(self.art_dataset_train)
        random.shuffle(self.art_dataset_test)
        # self.dataset = images
        print('Finished preprocessing the Art Works dataset, total image number: %d...'%len(self.art_dataset_test if self.mode == 'test' else self.art_dataset_train))
    ...
    def __getitem__(self, index):
        ...
        dataset = self.art_dataset_test if self.mode == 'test' else self.art_dataset_train
...
def getLoader(s_image_dir,c_image_dir, 
                ...
                crop_size=178, batch_size=16, num_workers=8, mode='train',
    ...
    style_dataset = StyleDataset(s_image_dir, style_selected_dir,s_transforms,mode=mode)
    style_data_loader = data.DataLoader(dataset=style_dataset,batch_size=batch_size,
                    drop_last=True,shuffle=True,num_workers=num_workers,pin_memory=True)
    prefetcher = data_prefetcher(style_data_loader)
    return prefetcher
...
if __name__ == "__main__":
    ...
    style_class  = ["vangogh","picasso","samuel",'berthe-morisot','edvard-munch',
